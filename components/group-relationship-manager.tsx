...
export function GroupRelationshipManager({ groupId, isCreator, isAdmin }: GroupRelationshipManagerProps) {
  ...
  const relevantRelationships = groupRelationships.filter(
    (rel) => rel.sourceGroupId === groupId || rel.targetGroupId === groupId,
  )
  ...
  // Get related groups
  const relatedGroups = groups.filter((g) => relatedGroupIds.has(g.id))

  // Get groups that don't have a relationship with this group yet
  const availableGroups = groups.filter(
    (g) => g.id !== groupId && !relatedGroupIds.has(g.id) && !g.parentGroup && !currentGroup.parentGroup,
  )
  ...
  const getRelationshipType = (group: Group): string => {
    if (group.parentGroup === groupId) return "subgroup"
    if (currentGroup.parentGroup === group.id) return "parent"
  ...
  const handleRemoveRelationship = (relatedGroupId: string) => {
    ...
    alert(
      `Relationship would be removed between ${currentGroup.name} and ${groups.find((g) => g.id === relatedGroupId)?.name}`,
    )
  }
          ...
          <Dialog open={showAddRelationship} onOpenChange={setShowAddRelationship}>
            ...
            <DialogContent>
              ...
              <div className="space-y-4 py-4">
                ...
                <div>
                  <label htmlFor="groupSearch" className="block text-sm font-medium text-gray-700 mb-1">
                    Search for a Group
                  </label>
                ...
                <div>
                  <label htmlFor="relationshipDescription" className="block text-sm font-medium text-gray-700 mb-1">
